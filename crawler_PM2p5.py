...
'''
This code is especially designed for feature PM2p5.
The feature name parameter is not needed.
'''
FEATURE_NAME = 'PM2.5'
...
if __name__ == "__main__":
    ...
    query_site_sect = soup_initial.select('select#ctl08_lbSite > option')
    query_site_list = [int(term['value']) for term in query_site_sect]
    query_feature_sect = soup_initial.select('select#ctl08_lbParam > option')
    ...
    payload = {
        ...
        'ctl08$lbSite': query_site_list, #觀測站
        'ctl08$lbParam': feature_param, #feature參數
        'ctl08$txtDateS': eta_time.strftime("%Y/%m/%d"),
        'ctl08$txtDateE': eta_time.strftime("%Y/%m/%d"),
